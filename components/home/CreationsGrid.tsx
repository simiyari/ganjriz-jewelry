      ...
      <div className="mt-10 grid grid-cols-2 gap-x-4 gap-y-8 sm:gap-x-6 lg:grid-cols-4">
        {CATEGORIES.map((cat) => (
          <Link key={cat.slug} href={`/category/${cat.slug}`} className="group flex flex-col">
            ...
            <span className="mt-4 text-center text-sm font-medium text-ink">
              <span className="relative inline-block pb-1">
                {cat.title}
                <span className="absolute inset-x-0 bottom-0 h-px origin-right scale-x-0 bg-ink transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-x-100" />
              </span>
            </span>
          </Link>
        ))}
